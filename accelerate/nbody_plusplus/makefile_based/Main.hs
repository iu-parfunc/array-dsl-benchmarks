...
-- Temporary: for debugging we aren't using a file at all:
  ...
  putStrLn$ "  Input prefix(4) "++ show(P.take 3$ A.toList $ I.run input)
      ...
      velo   = A.fill (shape input) (constant (0,0,0))
...
outerLoop :: A.Acc (A.Vector Position) -> A.Acc (A.Vector Velocity) -> 
             Int -> A.Acc (A.Vector Velocity)
outerLoop bodies velo n 
  = if n == 0 then velo
    else let accels  = calcAccels bodies
             velo'   = A.zipWith plusV accels velo
             bodies' = A.zipWith plusV velo' bodies
         in outerLoop bodies' velo' $ n - 1
